...
def evaluate_model_scale_four(model, test_image_paths):
    ...
    for i, path in zip(range(len(test_image_paths)), test_image_paths):
        ...
        psnr_acc.append(
            psnr(np.array(original), super_resolved.reshape([224, 224, 3])))  # cv2.PSNR(img1, img2) # tf.image.psnr
        print(psnr(np.array(original), super_resolved.reshape([224, 224, 3])))
    return tf.reduce_mean(psnr_acc)
