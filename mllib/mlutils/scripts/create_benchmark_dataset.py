...
def get_lr_and_hr_bicubic(image, dsize_lr=(112, 112), dsize_hr=(224, 224)):
...
def create_benchmark_dataset(dataset_folder, output_folder, malignant_samples=500, benign_samples=1500):
    ...
    for benign_image_path in benign:
        ...
        lr_png.save(os.path.join(output_folder, "benign", "lr", f"{image_name}.png"))

        hr_bicubic_png = Image.fromarray(hr_bicubic)
        hr_bicubic_png.save(os.path.join(output_folder, "benign", "hr_bicubic", f"{image_name}.png"))
    ...
    for malignant_image_path in malignant:
        ...
        lr, hr_bicubic = get_lr_and_hr_bicubic(image)
        lr_png = Image.fromarray(lr)
        lr_png.save(os.path.join(output_folder, "malignant", "lr", f"{image_name}.png"))

        hr_bicubic_png = Image.fromarray(hr_bicubic)
        hr_bicubic_png.save(os.path.join(output_folder, "malignant", "hr_bicubic", f"{image_name}.png"))
